...
import type { TestContent } from "@/types/courseContent";
...
const EditTestDrawer = ({
  ...
  const [testDurationSeconds, setTestDurationSeconds] = useState<string>(
    String(Math.round(test.test_duration / 60))
  );
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-[#4b5563]">Modify the test details</p>
        </div>
        <X
          onClick={onClose}
          className="h-4 w-4 cursor-pointer hover:text-red-500 duration-150 ease-in-out transition-all shrink-0"
        />
      </div>
